...
Créer un fichier `requirements.txt` contenant `tdp-lib` en dépendance :
...
- `display_skipped_hosts` (**recommandé**) permet de ne pas afficher les tâches Ansible qui sont _skipped_ pour éviter d'encombrer les logs.
...
- `realm` est le nom du _realm_ Kerberos.
- `kerberos_admin*`: TODO
- `kadmin*`: TODO
