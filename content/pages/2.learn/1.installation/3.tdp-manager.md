...
# Installation de TDP Manager

TDP Manager est un orchestrateur basé sur [Ansible](https://www.ansible.com/) permettant de déployer la stack TDP.

Note: Même si il apporte de nombreuses fonctionnalités, l'utilisation de TDP Manager n'est pas obligatoire. TDP peut également être déployé [manuellement](#).

## Architecture

TDP Manager est composé de la manière suivante :

- `tdp-lib`, un SDK qui expose des objets qui permettent de planifier, exécuter (via [Ansible](https://www.ansible.com/)) et monitorer des déploiements basés sur un [DAG](https://fr.wikipedia.org/wiki/Graphe_orient%C3%A9_acyclique). `tdp-lib` embarque également une CLI d'administration permettant de gérer le cluster.
...
- Avoir configuré le [noeud de contrôle](#)
- Avoir configuré les [noeuds managés](#)
...
`tdp-lib` utilise la commande `ansible-playbook` pour exécuter les playbooks. Celle-ci doit pouvoir être appelée sans option et sans interaction de l'utilisateur (Ansible Vault, élévation de privilèges, etc.).
...
pip install -r "tdp-lib[visualization]@https://github.com/TOSIT-IO/tdp-lib/tarball/master"
```
...
- `TDP_DATABASE_DSN`, adresse de la base de données à utiliser. (Par exemple pour une base de données SQLite : `sqlite:///tdp.db`)
