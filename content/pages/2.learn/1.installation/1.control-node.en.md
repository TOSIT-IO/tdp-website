...
; Configuration for the `tdp_vars` plugin (responsible of buildind the variables used by TDP's Ansible collections).
; This configuration may be OPTIONAL if values are passed through environment variables.
[tdp]
vars = tdp_vars ; OPTIONAL if the TDP_VARS environment variable is specified. Path to the tdp_vars directory initialized using the tdp CLI.
